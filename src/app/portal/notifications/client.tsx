...
import { usePortalNotificationsStream } from "@/lib/hooks/use-portal-notifications-stream";
import type { PortalNotificationPayload } from "@/lib/hooks/use-portal-notifications-stream";
...
export function PortalNotificationsClient({ initialItems }: { initialItems: Item[] }) {
  ...
  const onLiveNotification = useCallback((payload: PortalNotificationPayload) => {
    setItems((prev) => {
      if (prev.some((i) => i.id === payload.id)) return prev;
      const next: Item = {
        id: payload.id,
        event: payload.event,
        eventLabel: EVENT_LABELS[payload.event] ?? payload.event,
        title: payload.title,
        body: payload.body,
        link: payload.link,
  ...
  usePortalNotificationsStream(onLiveNotification);
  ...
  return (
    <div className="mt-8 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          읽지 않음 <span className="text-gold-deep">{unread}</span>건
        </span>
        {unread > 0 && (
          <button
            type="button"
...
function PortalNotificationsAnimStyle() {
  return (
    <style>{`
      @keyframes portalNotifFadeInSlide {
        from { opacity: 0; transform: translateY(-8px); }
        to   { opacity: 1; transform: translateY(0); }
      }
      .portal-notif-fresh {
        animation: portalNotifFadeInSlide 420ms ease-out both;
      }
      @media (prefers-reduced-motion: reduce) {
        .portal-notif-fresh { animation: none; }
      }
    `}</style>
  );
}
