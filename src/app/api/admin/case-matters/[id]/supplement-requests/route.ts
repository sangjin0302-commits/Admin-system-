...
import { createSupplementRequestSchema } from "@/lib/validation/case-matter";

export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const api = createAdminRequestContext("admin.case-matters.supplement-requests.post");
  ...
  try {
    ...
    const payload = createSupplementRequestSchema.parse(bodyResult.body);
    ...
    if (error instanceof SupplementRequestCreateError) {
    ...
    api.logError(error);
    return api.error(500, "Failed to create supplement request.", {
      code: "POST_SUPPLEMENT_REQUEST_FAILED"
    });
  }
}
