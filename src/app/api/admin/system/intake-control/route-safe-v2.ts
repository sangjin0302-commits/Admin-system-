...
export async function GET(request: Request) {
  ...
  try {
    ...
    api.logError(error);
    return api.error(500, "접수 운영 상태를 불러오지 못했습니다.", {
...
export async function POST(request: Request) {
  ...
  try {
    const capabilities = getPublicIntakeControlCapabilities();
    if (!capabilities.writable) {
      return api.error(503, capabilities.reason ?? "점검 모드 설정을 저장할 수 없습니다.", {
        code: "INTAKE_CONTROL_READ_ONLY"
      });
    }
    ...
    const bodyResult = await safeReadJsonBody(request);
    if (!bodyResult.ok) {
      return api.error(400, "요청 본문이 올바른 JSON 형식이 아닙니다.", {
        code: "INVALID_JSON_BODY"
      });
    }
    ...
    if (hasMeaningfulChange && !parsed.changeReason?.trim()) {
      return api.error(400, "운영 변경 사유를 입력해 주세요.", {
        code: "INTAKE_CONTROL_REASON_REQUIRED"
      });
    }
    ...
    if (error instanceof ZodError) {
      return api.error(400, firstZodMessage(error, "입력 값을 다시 확인해 주세요."), {
        code: "INVALID_INTAKE_CONTROL_INPUT"
      });
    }

    api.logError(error);
    return api.error(500, "접수 운영 상태를 저장하지 못했습니다.", {
      code: "UPDATE_INTAKE_CONTROL_FAILED"
    });
  }
}
