...
const ROWS: { label: string; env: string; get: () => EnvShape; note?: string }[] = [
  ...
  { label: "Lawbot 분석 URL (관리자) ★", env: "LAWBOT_ANALYZE_URL", get: () => describe(process.env.LAWBOT_ANALYZE_URL), note: "끝을 /analyze/admin 으로. 사건분석+quick-check 공용" },
  { label: "Lawbot 분석 URL (고객, 선택)", env: "LAWBOT_ANALYZE_PUBLIC_URL", get: () => describe(process.env.LAWBOT_ANALYZE_PUBLIC_URL), note: "미설정 시 위 URL에서 /analyze 로 자동 변환" },
