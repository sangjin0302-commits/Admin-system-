...
assert.match(source, /formatCompactChecklistValue/);
assert.match(source, /출처 검증 체크/);
...
assert.match(source, /고위험 서식 검토 필요/);
assert.match(source, /문서 생성 단계 도달을 의미하지 않습니다/);
assert.match(source, /현재 표시할 공식 출처 검토 항목이 없습니다/);
assert.match(source, /primaryReasonLabelKo/);
assert.match(source, /missingReasons/);
