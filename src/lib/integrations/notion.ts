...
function buildNeededDocuments(input: SyncCaseAnalysisInput) {
  ...
  return [...new Set([...recommendedDocs, ...lawDocs])].join(", ") || "추가 검토 필요";
}

function buildMissingDocuments(input: SyncCaseAnalysisInput) {
  return input.analysis.missingFacts.join(", ") || "원문 명시 없음";
}

function buildNextAction(input: SyncCaseAnalysisInput) {
  return input.analysis.recommendedAction || input.analysis.issues[0] || "사건 검토 후 다음 조치 확정";
}
...
function buildCaseBlocks(input: SyncCaseAnalysisInput) {
  ...
  return [
    ...baseBlocks,
    ...lawbotSections.flatMap(([heading, body]) => [
      {
        object: "block",
        type: "heading_2",
        heading_2: { rich_text: buildRichText(heading) },
...
export async function getNotionReferenceRecommendations(input: {
  ...
  const [archivePayload, websitePayload] = await Promise.all([
    archiveDatabaseId
