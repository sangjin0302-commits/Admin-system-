...
export const FEATURE_REGISTRY: readonly FeatureDefinition[] = [
  ...
  { key: "booking", label: "온라인 예약", category: "marketing", default: true, description: "온라인 상담 예약 기능", locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  ...
  { key: "trust_belt", label: "신뢰 뱃지 벨트 (언론에 소개된 ETHOS)", category: "marketing", default: false, description: "홈 '언론에 소개된 ETHOS' 로고 스트립 노출", public: true },
  { key: "local_landing_grid", label: "홈 지역별 안내 그리드", category: "marketing", default: false, description: "홈 하단 지역별 행정사 안내(강남·서초 등) 그리드 노출", public: true },
  { key: "footer_local_links", label: "푸터 지역 서비스 링크", category: "marketing", default: false, description: "푸터 '지역 서비스' 열(지역별 랜딩 링크) 노출", public: true },
  { key: "nav_about", label: "메뉴: 소개", category: "marketing", default: true, description: "상단 메뉴 '소개' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_services", label: "메뉴: 분야", category: "marketing", default: true, description: "상단 메뉴 '분야' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_consult", label: "메뉴: 상담", category: "marketing", default: true, description: "상단 메뉴 '상담' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_quick_check", label: "메뉴: AI 진단", category: "marketing", default: true, description: "상단 메뉴 'AI 진단' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_ai_screen", label: "메뉴: 빠른 진단", category: "marketing", default: true, description: "상단 메뉴 '빠른 진단' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_cases", label: "메뉴: 활동", category: "marketing", default: true, description: "상단 메뉴 '활동' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_blog", label: "메뉴: 칼럼", category: "marketing", default: true, description: "상단 메뉴 '칼럼' 노출", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  { key: "nav_gazette", label: "메뉴: 관보", category: "marketing", default: true, description: "상단 메뉴 '관보' 노출(관보 게시판 진입)", public: true },
  ...
  { key: "ai_chatbot", label: "AI 챗봇", category: "marketing", default: true, description: "공개 페이지 AI 챗봇", public: true, locked: true, lockReason: "공개 페이지 상시 상담 진입점 — 버튼 노출 + 공개 플래그 API 계약 e2e 검증 잠금 2026-07-25" },
  ...
  { key: "haptic_feedback", label: "햅틱 피드백", category: "ux", default: true, description: "모바일 진동 피드백 (CTA·폼 성공 등)", public: true, locked: true, lockReason: "안정 UX 유틸 — e2e 44/44 그린 잠금 2026-07-22" },
  ...
  { key: "citation_verify_gate", label: "인용 검증 게이트", category: "operations", default: true, description: "AI 서면 발송 전 폐지 조문·미검증 판례 자동 차단 게이트", locked: true, lockReason: "AI 서면 법적 정확성 안전장치 잠금 2026-07-22" },
  ...
  { key: "dark_mode", label: "다크모드", category: "ux", default: true, description: "prefers-color-scheme 감지 + 수동 토글 (야간 작업 대응)" },
  ...
  { key: "dark_mode_manual_toggle", label: "다크모드 수동 토글", category: "ux", default: true, description: "라이트/자동/다크 3단 토글 UI (data-theme + localStorage)" },
  ...
  { key: "blog_auto_translate", label: "블로그 자동 번역 실행", category: "marketing", default: false, description: "네이버 수입글의 EN 자동번역(cron·수입시). 기본 OFF — 관리자가 영문 직접 입력. 직접작성글(source!=NAVER)은 어떤 경우에도 자동번역 안 함." },
  ...
  { key: "cms_image_upload", label: "CMS 이미지 업로드", category: "operations", default: true, description: "히어로 배경·인물·서비스 배경 이미지를 CMS에서 업로드/교체", locked: true, lockReason: "로고·사진·브랜드 스토리 이미지 직접 교체 핵심 기능 — 항상 활성 잠금 2026-07-22" },
  ...
  { key: "admin_top_nav_layout", label: "Admin 상단 2단 네비", category: "admin", default: true, description: "사이드바 대신 상단 2단 가로 네비게이션으로 콘텐츠 가로 공간 최대화", locked: true, lockReason: "상단 네비 클릭 이동 검증 완료 2026-07-21" },
  { key: "intake_form_autosave", label: "접수 폼 자동저장", category: "ux", default: true, description: "접수 폼 작성 중 자동 저장 및 복구 (localStorage)", public: true, locked: true, lockReason: "e2e 44/44 검증 완료 잠금 2026-07-21" },
  ...
  { key: "blog_category_cta", label: "블로그 카테고리별 CTA", category: "marketing", default: true, description: "블로그 글 하단에 카테고리 맞춤 CTA 카드 노출", public: true, locked: true, lockReason: "블로그 유입 → 검토요청 전환 경로 — 글 하단 CTA 렌더 e2e 검증 잠금 2026-07-25" },
  ...
  { key: "intake_kakao_share", label: "카카오톡 공유 버튼", category: "intake", default: true, description: "접수 완료 후 카카오톡 공유 버튼 노출", public: true, locked: true, lockReason: "접수 완료 전환 요소 — e2e 44/44 그린 잠금 2026-07-22" },
  ...
  { key: "intake_exit_guard", label: "접수 이탈 방지", category: "intake", default: true, description: "접수 폼 작성 중 브라우저 나가기 방지", public: true, locked: true, lockReason: "접수 이탈 방지 핵심 — e2e 44/44 그린 잠금 2026-07-22" },
  ...
  { key: "blog_related_posts", label: "관련 글 추천", category: "content", default: true, description: "블로그 글 하단에 같은 카테고리 관련 글 3개 추천", public: true, locked: true, lockReason: "블로그 체류·회유 안정 기능 — e2e 44/44 그린 잠금 2026-07-22" },
  { key: "public_law_search", label: "고객용 법령·판례 참고 검색", category: "content", default: true, description: "국가법령정보센터 제목 참고 검색 (IP당 일 3회, AI 처리 없음)", public: true, locked: true, lockReason: "법제처 실호출 검증 완료 2026-07-19 (status=ok, 출입국관리법 1건)" },
  ...
  { key: "admin_strict_rbac", label: "Admin 엄격 RBAC (백도어 차단)", default: true, category: "admin" as FeatureCategory, description: "X-Admin-User 헤더 스푸핑 및 미등록 Basic Auth 사용자 SUPER 자동 승격 차단", locked: true, lockReason: "관리자 인증 보안 백도어 차단 — 절대 해제 금지 잠금 2026-07-22" },
