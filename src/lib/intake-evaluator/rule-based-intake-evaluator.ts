...
export class RuleBasedIntakeEvaluator implements IntakeEvaluator {
  evaluate(input: IntakeEvaluationInput): IntakeEvaluationResult {
    ...
    if (!input.hasPreparedDocuments) hints.push("핵심 서류 미보유로 초기 상담 필요");
    if (input.needsTranslation) hints.push("번역 범위 확인 필요");
    if (input.isCorporateRequest || input.clientType === "COMPANY") hints.push("기업 의뢰: 의사결정 주체 및 범위 확인 필요");
    ...
    return {
      ...
      riskComplexityHint: hints.join(" / ") || "일반 난이도"
    };
  }
}
