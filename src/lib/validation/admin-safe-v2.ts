...
export const updateInquiryAdminSchema = z
  ...
  .refine(
    (value) =>
      ...
      value.internalMemo !== undefined ||
      value.statusChangeNote !== undefined,
    {
...
export const appendInquiryCommunicationLogSchema = z.object({
  ...
  summary: z
    .string()
    .trim()
    .min(2, "연락 요약을 입력해 주세요.")
    .max(400, "연락 요약은 400자 이내로 입력해 주세요."),
  details: z
    .string()
    .trim()
    .max(3000, "상세 메모는 3000자 이내로 입력해 주세요.")
    .optional()
    .default(""),
  ...
  nextContactAt: z
    ...
    .refine((value) => value === undefined || !Number.isNaN(new Date(value).getTime()), {
      message: "다음 연락 일정 형식이 올바르지 않습니다."
    })
});
