...
export function InquiryWorkQueues({
  groups
}: {
  groups: QueueGroup[];
}) {
  return (
    <Card className="p-6">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <p className="text-sm text-text-muted">상세 화면에 들어가기 전에 어떤 종류의 일이 몰려 있는지 먼저 보여줍니다.</p>
      </div>
            ...
            <div className="mt-4 space-y-3">
              {group.items.length > 0 ? (
                group.items.map((item) => (
                  <Link key={item.id} href={item.href} className="block rounded-xl border border-line bg-white px-3 py-3 transition hover:border-line-strong hover:bg-surface">
