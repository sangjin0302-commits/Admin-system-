...
export function BlogRelatedPosts({ posts }: { posts: RelatedPost[] }) {
  ...
  return (
    <section className="mt-16 border-t border-gold/20 pt-10">
      <h3 className="text-lg font-bold text-primary">관련 글</h3>
      <div className="mt-5 grid gap-4 sm:grid-cols-3">
        {posts.map(p => (
          <Link key={p.slug} href={`/blog/${p.slug}`} className="group rounded-xl border p-5 transition hover:border-gold/50 hover:shadow-sm">
            {p.category && <span className="text-xs font-bold text-gold-deep">{p.category}</span>}
